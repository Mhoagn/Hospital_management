...
class User(db.Model, UserMixin):
    ...
    def get_upcoming_examinations(self):
        today = func.current_date()
        return Examination.query.filter_by(user_id=self.user_id).filter(Examination.date >= today).all()
    
    def get_upcoming_examinations_for_doctor(self):
        today = func.current_date()
        # Truy vấn các cuộc khám sắp tới cho bác sĩ dựa trên ID của bác sĩ
        return Examination.query.filter_by(doctor_id=self.user_id).filter(Examination.date >= today).all()
